 ...
 * @date    Oct 12, 2020
 *
 * @brief Functions for fundamental array algorithms using a struct.
 *
 * @details Using a struct to hold all the functions implemented. Including 
 * basic counting, summing, searching, and basic statistical methods (average,
...
double midpoint(int a[], size_t size, int max, int min);
int linear_search(int a[], size_t size, int mid);
int get_sum(int a[], size_t size);
int get_count(int a[], size_t size, int type, int midpoint);
double average(int a[], size_t size, int sum);
void reverse(int a[], size_t size);
int even_count(int a[], size_t size);
int odd_count(int a[], size_t size);
int divisible_count(int a[], size_t size);
struct summary_t get_summary(int a[], size_t size, struct summary_t summary);
...
int main(void)
{
    ...
    //calls all functions 
    summary = get_summary(a, size, summary);
	
    //prints all functions 
    print_summary(a, size, summary);
   
	return 0;
}

/** calls all the functions from program and initializes struct
 * @param a[] the array to print
 * @param size the size or length of the array
 * @param summary the struct
 */
...
struct summary_t get_summary(int a[], size_t size, struct summary_t summary)
{
    ...
    summary.mid = midpoint(a, summary.size, summary.max, summary.min);
	summary.search = linear_search(a, size, summary.mid);
	summary.countLT = get_count(a, summary.size, LT, summary.mid);
    summary.countGT = get_count(a, summary.size, GT, summary.mid);
	summary.sum = get_sum(a, summary.size);
    summary.average = average(a, summary.size, get_sum(a, summary.size));
    summary.e_count = even_count(a, summary.size);
    summary.o_count = odd_count(a, summary.size);
    summary.d_count = divisible_count(a, summary.size);
	return summary;
}
...
/** prints out the returns of the rest of the program's functions
 * @param a[] the array to print
 * @param size the size or length of the array
 */
void print_summary(int a[], size_t size, struct summary_t summary)
{
	...
	printf ("min = %d\n", summary.min);
	printf ("midpoint = %f\n", summary.mid);
	printf ("less than than midpoint = %d\n", summary.countLT);
	printf ("greater than than midpoint = %d\n", summary.countGT);
	printf ("search for midpoint = %d\n", summary.search);
	printf ("sum = %d\n", summary.sum);
	printf ("average = %f\n", summary.average);
	printf ("even = %d\n", summary.e_count);
	printf ("odd = %d\n", summary.o_count);
	printf ("divisible by the minimum value of array = %d\n", summary.d_count);
...
/** finds the minimum value from array a
 * @param a[] the array to print
 * @param size the size or length of the array
...
/** finds the midoint value from array a
 * @param a[] the array to print
 * @param size the size or length of the array
 * @param max the max from find_max function
 * @param min the min from find_min function
 * @return midpoint of the array
 */
double midpoint(int a[], size_t size, int max, int min)
{
	double mid;
...
/** finds the define value from array a
 * @param a[] the array to print
 * @param size the size or length of the array
 * @return elements greater than the midpoint
 * @return elements less than the midpoint
 */
int get_count(int a[], size_t size, int type, int midpoint)
{
...
/** attempts to find the midpoint value from array a 
 * @param a[] the array to print
 * @param size the size or length of the array
 * @return index if element is found in array, -1 if not
 */
int linear_search(int a[], size_t size, int mid)
{
	int i, search; 
...
/** finds the sume of all elements from array a 
 * @param a[] the array to print
 * @param size the size or length of the array
...
/** finds the average of all elements from array a 
 * @param a[] the array to print
 * @param size the size or length of the array
 * @param sum the sum of the array
 * @return average of array elements (sum /2)
 */
double average(int a[], size_t size, int sum)
{
    return (double) sum / 2; 
...
/** finds the count of all elements with even numbers from array a 
 * @param a[] the array to print
 * @param size the size or length of the array
 * @return count of even numbered elements
 */
int even_count(int a[], size_t size)
{
	int i = 0;
...
/** finds the count of all elements with odd numbers from array a 
 * @param a[] the array to print
 * @param size the size or length of the array
 * @return count of odd numbered elements
 */ 
int odd_count(int a[], size_t size)
{
	int i = 0;
...
/** finds the count of all elements that are divisible by value x 
 * @param a[] the array to print
 * @param size the size or length of the array
